...
try:
    ...
    from paths import CHECK_RECORDINGS_SCRIPT, LIFE_RECORD_DIR, WORKSPACE_ROOT
except ImportError:
...
2. **深度调查日志**：
   - 对于每个任务，分析其执行频率。
   - 识别日志文件路径。
   - **向下钻取 (Drill-down)**：如果 Crontab 中没有明确的日志重定向（例如没有 `>>`），请**不要直接假设它没有日志**。你应该定位脚本文件（例如 `{workspace_root}/scripts/cron_launcher.sh`）并 `read` 脚本内容，检查其内部是否有日志重定向逻辑（例如 `exec >>`）。
3. **活跃度分析**：
   - 使用工具检查日志文件的最后修改时间。
...
5. **录音质量检查**：
   - 运行 life_record 的录音检查脚本，检查**最近两天**的录音是否有问题。
    - 脚本路径：`{check_recordings_script}`
   - 执行方式：先 `cd {life_record_dir}`，对最近两天的 YYYYMMDD 各运行一次。可通过 `ls data/` 获取最近两个日期目录，或使用 `date -v-1d +%Y%m%d` 获取昨天日期。示例：`python scripts/check_recent_recordings.py`（最近一天）及 `python scripts/check_recent_recordings.py 20260224`（指定日期）。
...
def run_ai_analysis():
    ...
    prompt = PROMPT_TEMPLATE.format(
        ...
        check_recordings_script=CHECK_RECORDINGS_SCRIPT,
        life_record_dir=LIFE_RECORD_DIR,
    )
